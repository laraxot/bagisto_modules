...
@section('page_title')
    {{ __('myTestTheme::app.admin.contents.add-title') }}
@stop
    ...
    <div class="content">
        ...
        <form
            ...
            <div class="page-header">
                    ...
                    <div class="control-group">
                        <select class="control" id="locale-switcher" onChange="window.location.href = this.value">
                            @foreach (core()->getAllLocales() as $localeModel)
                                ...
                                <option value="{{ route('myTestTheme.admin.content.update', $content->id) . '?locale=' . $localeModel->code }}" {{ ($localeModel->code) == $locale ? 'selected' : '' }}>
                                    {{ $localeModel->name }}
                                </option>
                ...
                <div class="page-action">
                    <button type="submit" class="btn btn-lg btn-primary">
                        {{ __('myTestTheme::app.admin.contents.save-btn-title') }}
                    </button>
                </div>
            </div>
            ...
            <div class="page-content">
                ...
                <accordian :title="'{{ __('myTestTheme::app.admin.contents.tab.page') }}'" :active="true">
                    <div slot="body">
                        ...
                        <div class="control-group" :class="[errors.has('{{$locale}}[title]') ? 'has-error' : '']">
                            <label for="title" class="required">
                                {{ __('myTestTheme::app.admin.contents.page.title') }}
                                <span class="locale">[{{ $locale }}]</span>
                            </label>
                            <input type="text" v-validate="'required|max:100'" class="control" id="title" name="{{$locale}}[title]" value="{{ old($locale)['title'] ?? isset($translation->title) ? $translation->title : '' }}" data-vv-as="&quot;{{ __('myTestTheme::app.admin.contents.page.title') }}&quot;"/>
                        ...
                        <div class="control-group" :class="[errors.has('position') ? 'has-error' : '']">
                            <label for="position" class="required">
                                {{ __('myTestTheme::app.admin.contents.page.position') }}</span>
                            </label>
                            <input type="text" v-validate="'required|numeric|max:2'" class="control" id="position" name="position" value="{{ old('position') ?? $content->position }}" data-vv-as="&quot;{{ __('myTestTheme::app.admin.contents.page.position') }}&quot;"/>
                            <span class="control-error" v-if="errors.has('position')">@{{ errors.first('position') }}</span>
                        </div>

                        <div class="control-group" :class="[errors.has('status') ? 'has-error' : '']">
                            <label for="status" class="required">{{ __('myTestTheme::app.admin.contents.page.status') }}</label>
                            <select class="control" v-validate="'required'" id="status" name="status" data-vv-as="&quot;{{ __('myTestTheme::app.admin.contents.page.status') }}&quot;">
                                <option value="0" {{ !$content->status ? 'selected' : '' }}>
                                    {{ __('myTestTheme::app.admin.contents.inactive') }}
                                </option>
                                <option value="1" {{ $content->status ? 'selected' : '' }}>
                                    {{ __('myTestTheme::app.admin.contents.active') }}
                                </option>
                            </select>
                ...
                <accordian :title="'{{ __('myTestTheme::app.admin.contents.tab.content') }}'" :active="true">
                    <div slot="body">
        ...
        <div>
            {{-- <div :class="`control-group ${errors.has('{{$locale}}[custom_title]') ? 'has-error' : ''}`">
                <label for="custom_title">
                    {{ __('myTestTheme::app.admin.contents.content.custom-title') }}
                    <span class="locale">[{{ $locale }}]</span>
                </label>
                ...
                <input
                    ...
                    data-vv-as="&quot;{{ __('myTestTheme::app.admin.contents.content.custom-title') }}&quot;" />
            ...
            <div :class="`control-group ${errors.has('{{$locale}}[custom_heading]') ? 'has-error' : ''}`">
                <label for="custom_heading">
                    {{ __('myTestTheme::app.admin.contents.content.custom-heading') }}
                    <span class="locale">[{{ $locale }}]</span>
                </label>
                ...
                <input
                    ...
                    value="{{ old($locale)['custom_heading'] ?? $content->translate($locale) ? $content->translate($locale)['custom_title'] : '' }}" data-vv-as="&quot;{{ __('myTestTheme::app.admin.contents.content.custom-heading') }}&quot;" />
            ...
            <div :class="`control-group ${errors.has('content_type') ? 'has-error' : ''}`">
                <label
                    for="content_type"
                    class="required">
                    {{ __('myTestTheme::app.admin.contents.content.content-type') }}
                </label>
                ...
                <select
                    ...
                    v-validate="'required'"
                    data-vv-as="&quot;{{ __('myTestTheme::app.admin.contents.content.content-type') }}&quot;" @change="loadFields($event)">

                    <option value="">
                        {{ __('myTestTheme::app.admin.contents.select') }}
                    </option>

                    @foreach (myTestTheme()->getContentType() as $key => $content_type)
                        <option
                            value="{{ $key }}"
            ...
            <div v-if="content_type == 'link'">
                ...
                <div :class="`control-group ${errors.has('{{$locale}}[page_link]') ? 'has-error' : ''}`">
                    ...
                    <label for="page_link" class="required">
                        {{ __('myTestTheme::app.admin.contents.content.page-link') }}
                    </label>
                    ...
                    <input
                        ...
                        data-vv-as="&quot;{{ __('myTestTheme::app.admin.contents.content.page-link') }}&quot;" />
                ...
                <div class="control-group">
                    <label for="link_target">
                        {{ __('myTestTheme::app.admin.contents.content.link-target') }}
                    </label>
                    ...
                    <select
                        ...
                        <option value="0" {{ !$content->link_target ? 'selected' : '' }}>
                            {{ __('myTestTheme::app.admin.contents.self') }}
                        </option>
                        <option value="1" {{ $content->link_target ? 'selected' : '' }}>
                            {{ __('myTestTheme::app.admin.contents.new-tab') }}
                        </option>
                    </select>
                </div>
            ...
            <div v-else-if="content_type == 'product'">
                @include ('myTestTheme::admin.content.content-type.edit-product')
            </div>
            ...
            <div v-else-if="content_type == 'static'">
                ...
                <div :class="`control-group ${errors.has('{{$locale}}[description]') ? 'has-error' : ''}`">
                    <label for="description" class="required">{{ __('myTestTheme::app.admin.contents.content.static-description') }}</label>
                    ...
                    <textarea
                        ...
                        data-vv-as="&quot;{{ __('myTestTheme::app.admin.contents.content.static-description') }}&quot;">
            ...
            <div v-else-if="content_type == 'category'">
                @include ('myTestTheme::admin.content.content-type.category')
            </div>
        </div>
    </script>

        ...
        <div>
            ...
            <div class="control-group" :class="[errors.has('{{$locale}}[catalog_type]') ? 'has-error' : '']">
                <label for="catalog_type" class="required">
                    {{ __('myTestTheme::app.admin.contents.content.catalog-type') }}</label>

                <select class="control" v-validate="'required'" id="catalog_type" name="{{$locale}}[catalog_type]" v-model="catalog_type" data-vv-as="&quot;{{ __('myTestTheme::app.admin.contents.content.catalog-type') }}&quot;" @change="loadCatalogType($event)">
                    <option value="">{{ __('myTestTheme::app.admin.contents.select') }}</option>

                    @foreach (myTestTheme()->getCatalogType() as $key => $catalog_type)
            ...
            <div v-if="catalog_type == 'custom'">
                <field-autocomplete
                    ...
                    :fieldPlaceholder="'{{ __('myTestTheme::app.admin.contents.search-hint') }}'"
                    :routePath="'{{ route('myTestTheme.admin.content.search') }}'"
                    :linkedResults='@json(app('Modules\myTestTheme\Repositories\ContentRepository')->getProducts($content->id))'
                ></field-autocomplete>
            </div>
        </div>
                ...
                <div class="linked-product-search-result">
                    <ul>
                        ...
                        <li v-if='! searched_results.length && search_field.length && ! is_searching'>
                            {{ __('myTestTheme::app.admin.contents.no-result-found') }}
                        </li>

@php
    $attributeRepository = app('\Modules\Attribute\Repositories\AttributeFamilyRepository');
    $comparableAttributes = $attributeRepository->getComparableAttributesBelongsToFamily();

    $locale = request()->get('locale') ?: app()->getLocale();

    $attributeOptionTranslations = app('\Modules\Attribute\Repositories\AttributeOptionTranslationRepository')->where('locale', $locale)->get()->toJson();
@endphp
...
@push('scripts')
    <script type="text/x-template" id="compare-product-template">
        <section class="cart-details row no-margin col-12">
            <h2 class="fw6 col-6">
                {{ __('myTestTheme::app.customer.compare.compare_similar_items') }}
            </h2>
                ...
                <template v-else-if="isProductListLoaded && products.length > 0">
                    @php
                        ...
                        array_splice($comparableAttributes, 1, 0, [[
                            'code' => 'product_image',
                            'admin_name' => __('myTestTheme::app.customer.compare.product_image'),
                        ]]);

                        array_splice($comparableAttributes, 2, 0, [[
                            'code' => 'addToCartHtml',
                            'admin_name' => __('myTestTheme::app.customer.compare.actions'),
                        ]]);
                    @endphp
                                            ...
                                            @case('boolean')
                                                <span
                                                    v-text="product.product['{{ $attribute['code'] }}']
                                                            ? '{{ __('myTestTheme::app.shop.general.yes') }}'
                                                            : '{{ __('myTestTheme::app.shop.general.no') }}'"
                                                ></span>
                                                @break;

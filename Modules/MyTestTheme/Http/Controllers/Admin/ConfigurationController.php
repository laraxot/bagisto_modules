...
{
    /**
     * myTestThemeMetadataRepository object
     *
     ...
     *
     * @param  \Modules\myTestTheme\Repositories\MetadataRepository  $myTestThemeMetaDataRepository
     * @return void
     */
    public function __construct (myTestThemeMetadataRepository $myTestThemeMetadataRepository)
    {
        $this->_config = request('_config');

        $this->myTestThemeHelper = app('Modules\myTestTheme\Helpers\Helper');
    ...
    {   
        ...
        $myTestThemeMetaData = $this->myTestThemeHelper->getmyTestThemeMetaData($this->locale, $this->channel, false);

        if (! $myTestThemeMetaData) {
            $this->createMetaData($this->locale, $this->channel);

            $myTestThemeMetaData = $this->myTestThemeHelper->getmyTestThemeMetaData($this->locale, $this->channel);
        }

        $myTestThemeMetaData->advertisement = $this->manageAddImages(json_decode($myTestThemeMetaData->advertisement, true) ?: []);

        return view($this->_config['view'], [
            'metaData' => $myTestThemeMetaData,
        ]);
    }
    ...
    public function storeMetaData($id)
    {
        ...
        $myTestThemeMetaData = $this->myTestThemeMetaDataRepository->findOneWhere([
            'id' => $id,
        ]);

        $advertisement = json_decode($myTestThemeMetaData->advertisement, true);
        ...
        // update row
        $product = $this->myTestThemeMetaDataRepository->update($params, $id);

        session()->flash('success', trans('admin::app.response.update-success', ['name' => trans('myTestTheme::app.admin.meta-data.title')]));
    ...
    {
        \DB::table('myTestTheme_meta_data')->insert([
            'locale'                   => $locale,
            'channel'                  => $channel,
            ...
            'footer_left_content'      => __('myTestTheme::app.admin.meta-data.footer-left-raw-content'),

...
class ComparisonController extends Controller
{
            ...
            if (request()->get('data')) {
                ...
                if (auth()->guard('customer')->user()) {
                    $productCollection = $this->compareProductsRepository
                        ->leftJoin(
                            'product_flat',
                            'myTestTheme_customer_compare_products.product_flat_id',
                            'product_flat.id'
                        )
                    ...
                    $productCollection = ! empty($items)
                        ? $this->myTestThemeHelper->fetchProductCollection($items)
                        : [];
                } else {
                    /* for product details */
                    if ($items = request()->get('items')) {
                        $productCollection = $this->myTestThemeHelper->fetchProductCollection($items);
                    }
                }
    ...
    public function addCompareProduct()
    {
        ...
        if (! $compareProduct) {
            ...
            $productFlatRepository = app('\Modules\Product\Models\ProductFlat');
            ...
            return response()->json([
                'status'  => 'success',
                'message' => trans('myTestTheme::app.customer.compare.added'),
            ...
            return response()->json([
                'status'  => 'success',
                'label'   => trans('myTestTheme::app.shop.general.alert.success'),
                'message' => trans('myTestTheme::app.customer.compare.already_added'),
            ], 200);
        }
    }
    ...
    public function deleteComparisonProduct()
    {
        ...
        if (request()->get('productId') == 'all') {
            ...
            $message = trans('myTestTheme::app.customer.compare.removed-all');
        } else {
            // delete individual
            $this->compareProductsRepository->deleteWhere([
            ...
            $message = trans('myTestTheme::app.customer.compare.removed');
        }

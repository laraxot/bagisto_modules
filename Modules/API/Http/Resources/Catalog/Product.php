...
namespace Modules\API\Http\Resources\Catalog;
...
class Product extends JsonResource
{
    ...
    {
        $this->productReviewHelper = app('Modules\Product\Helpers\Review');

        $this->wishlistHelper = app('Modules\Customer\Helpers\Wishlist');
    }
    ...
    private function allProductExtraInfo()
    {
        ...
        return [
            /* grouped product */
            $this->mergeWhen(
                $productTypeInstance instanceof \Modules\Product\Type\Grouped,
                $product->type == 'grouped'
            ...
            /* bundle product */
            $this->mergeWhen(
                $productTypeInstance instanceof \Modules\Product\Type\Bundle,
                $product->type == 'bundle'
            ...
            $this->mergeWhen(
                $productTypeInstance instanceof \Modules\Product\Type\Configurable,
            ...
            $this->mergeWhen(
                $productTypeInstance instanceof \Modules\Product\Type\Downloadable,
     ...
     *
     * @param  \Modules\Product\Models\Product
     * @return array
     */
    ...
    private function getBundleProductInfo($product)
    {
        return [
            ...
            'bundle_options' => app('Modules\Product\Helpers\BundleOption')->getBundleConfig($product)
        ];
    }
    ...
    {
        $bookingProduct = app('\Modules\BookingProduct\Repositories\BookingProductRepository')->findOneByField('product_id', $product->id);

        $data['slot_index_route'] = route('booking_product.slots.index', $bookingProduct->id);

        if ($bookingProduct->type == 'appointment') {
            $bookingSlotHelper = app('\Modules\BookingProduct\Helpers\AppointmentSlot');
        ...
        if ($bookingProduct->type == 'event') {
            $bookingSlotHelper = app('\Modules\BookingProduct\Helpers\EventTicket');
        ...
        if ($bookingProduct->type == 'table') {
            $bookingSlotHelper = app('\Modules\BookingProduct\Helpers\TableSlot');
